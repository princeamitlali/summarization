...
import torch.nn.functional as F                                                                        #the above are required file to import
sys.setrecursionlimit(100000)                    #at a particular time our code went into infinite loop so we set the recursion limit
try:
    import ipdb
except ImportError:
    pass
...
import xargs
from PyRouge.Rouge import Rouge                       #here files are imported from another python script and feeded into exceptional handler
...
opt = parser.parse_args()                             #as train.py has all arguments and it will feed forward to all so it parse the required argunemt and feed them furthur
...
logger.info(opt)                                        #it give name to the logfile depending on date and time concatenated with log.txt
...
logger.info('My cuda seed is {0}'.format(torch.cuda.initial_seed()))                           #it will find the gpu and information about it and save in log file


def regression_loss(pred_scores, gold_scores, mask, crit):                           #calculate the  regression loss
    """
...
def load_dev_data(summarizer, src_file, tgt_file):                               #this will load the validation daafiles strip the into lines whereever ##SENT## delimiter are present
    ...
    for sline, tline in addPair(srcF, tgtF):
        if (sline is not None) and (tline is not None):
            ...
            if len(src_batch) < opt.batch_size:                                   #then make batch of files by appending them together
                continue
        else:
            # at the end of file, check last batch
            if len(src_batch) == 0:
                break
        data = summarizer.buildData(src_batch, src_raw, tgt_raw, None, None)        #here call the summerizerfile and builddata codeblock from there where they append
...
evalModelCount = 0
totalBatchCount = 0
rouge_calculator = Rouge.Rouge(use_ngram_buf=True)                                #here is the initialization for batch model and rouge calculator


def evalModel(model, summarizer, evalData):                  #this code clock do evaluation of model by calculating prediction score and gold score 
    ...
    for data in dataset:
        ...
        #  (2) translate
        batch = data[0]
        pred, predScore, attn, _ = summarizer.translateBatch(batch)               #summarizer script has translate batch which return hyper barameter score and attn parameter for same
        ...
        for b in range(len(src_raw)):
            n = 0
            selected_sents = []                                                 #in our output every sentence is given an id so in here the output is generated according to idx and then append
            selected_id = []
            for idx in pred[b][n]:
                # if idx == 0 or idx - 1 >= len(src_raw[b]):                    #this is done first for an article then for a batch
                #     break
        ...
        gold += tgt_raw
        predict += predBatch
    scores = rouge_calculator.compute_rouge(gold, predict)                         #this calculate rouge 2 f1 score for prediction and gold
    ...
    return scores['rouge-2']['f']
                                                                                       #this will print a rouge 2 score for each batch

def gen_pointer_mask(target_lengths, batch_size, max_step):
    res = torch.ByteTensor(batch_size, max_step).fill_(1)                            #this codeblock is used to create a tensor of byte data type with default value 1
...
def trainModel(model, summarizer, trainData, validData, dataset, optim):                #here the training starts
    logger.info(model)
    model.train()
    logger.warning("Set model to {0} mode".format('train' if model.training else 'eval'))     #save in log filemodel is training or evaluating

    # define criterion of each GPU
    regression_crit = nn.KLDivLoss(reduction='none')                                     #kldivergence is called which is used furthur

    def saveModel(metric=None):                                                          #this code block will save model and create checkpoint the model is saved only when the rouge score is better
    ...
    def trainEpoch(epoch):                                    #this run on starting traing of an epoch this will shuffle data and then start training for an epoch
        ...
        batchOrder = torch.randperm(len(trainData))         #randomly shuffle the data

        total_reg_loss = total_point_loss = total_docs = total_points = 0    #initialize everything to zero
        start = time.time()
        for i in range(len(trainData)):
            global totalBatchCount
            totalBatchCount += 1                                         #increase batch count in an epoch
            """
            ...
            # regression loss call from above  
        ...
        return total_reg_loss / total_docs                                                   #the above  will update parameter and save the model if applicable
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        if opt.dump_epoch_checkpoint:
            ...
            saveModel()                                                                    #this will save checkpoint after each epoch irrespective of any condition as the data is need to be shuffle


def main():                                                      #main function for this script import onlinePreprocess
    if not opt.online_process_data:
        ...
        dataset = prepare_data_online(opt.train_src, opt.src_vocab, opt.train_tgt, opt.tgt_vocab, opt.train_oracle,
                                      opt.train_src_rouge)                                                              #argument are feeded
    ...
    logger.info('Building model...')                                                  #log file updated 

    sent_encoder = neusum.Models.Encoder(opt, dicts['src'])                         #sentence level encoading are done it is of without attn
    doc_encoder = neusum.Models.DocumentEncoder(opt)                                #document level encoading are done with attn
    pointer = neusum.Models.Pointer(opt, dicts['tgt'])                              #all 3 are from neusum>models.py
    if opt.dec_init == "simple":
        decIniter = neusum.Models.DecInit(opt)                                      
    elif opt.dec_init == "att":
        decIniter = neusum.Models.DecInitAtt(opt)
    else:
        raise ValueError('Unknown decoder init method: {0}'.format(opt.dec_init))     #the decoder is initialized accordingly simple means without attn

    model = neusum.Models.NMTModel(sent_encoder, doc_encoder, pointer, decIniter, rouge_calculator)  #encoding decoding is on work
    summarizer = neusum.Summarizer(opt, model, dataset)                                 #summary is created
    ...
    for pr_name, p in model.named_parameters():
        ...
        if p.dim() == 1:
            ...
            # xavier_uniform(p)
    sent_encoder.load_pretrained_vectors(opt, logger)                             #user warning and loading pretrained vector as glove
    ...
    optim = neusum.Optim(
        ...
        decay_bad_count=opt.halve_lr_bad_count
                                                                                       ...
                                                                                       #optim script is called with argument
    optim.set_parameters(model.parameters())                                             #parameter is feeded into it
