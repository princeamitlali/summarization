...
class ScoreAttention(nn.Module):
    ...
    def forward(self, attend_dim, query_dim, att_dim):
        ...
        #xe , xp =[attn(linear_pre,linear_q,linear_v)
         ...
         print(heads,d_k,bs)
         print("..............................................................")
         #k = self.k_linear(k).view(bs, -1, self.h, d_k)
         ...
         energy = xe.transpose(1,2).contiguous().view(bs, -1, d_model)                                    #torch.cat(xe,dim = linear_q)
         precompute = xp.transpose(1,2).contiguous().view(bs, -1, d_model)                                #torch.cat(xp,dim = linear_v)
       # x= self.projection(x)
